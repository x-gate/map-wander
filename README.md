...
以 PixiJS 8 與同工作區的 `xglib` WASM，在瀏覽器中把動畫角色放進 `1011.dat`，試驗原版風格的游標、格位提示、八方向動畫與右鍵移動。
...
- 移動滑鼠：原版圖像 ID 1899 取代作業系統游標；有效地圖格以 ID 1900 覆蓋提示。
- 右鍵點擊格位：以 A* 尋找八方向路徑，角色切換至動作 1 並依移動方向播放；抵達後切回動作 0。
- 左鍵拖曳：平移地圖。
- 滾輪：以游標為中心縮放。

方向遵循這個專案的明確契約：0 西北、1 北，之後順時針至 7 西。動畫 ID 100052 在目前參考索引中有同號資料；載入器固定採第一筆索引列並在畫面標示來源，避免無聲覆蓋。
