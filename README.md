...
可行走網格以 `GraphicInfo.access` 建立：缺少或不可通行的地面不能進入；已知且不是 `AsGround` 的不可通行物件會依 `grid_w` × `grid_h` 封鎖完整占地，而非只封鎖物件錨點。`1011.dat` 的物件 ID 2 在 `GraphicInfo_66` 缺少圖像，但跨資源候選的碰撞欄位一致，因此維持透明並視為一格不可穿越障礙。這是試驗性相容策略，不宣稱已完整還原原版網路校正或移動速度。
